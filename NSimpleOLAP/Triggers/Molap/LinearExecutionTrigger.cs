...
{
  internal class LinearExecutionTrigger<T> : AbstractMolapTrigger<T>
    where T : struct, IComparable
  {
    private TriggerHelper<T, ICell<T>> _triggerHelper;
    public LinearExecutionTrigger(TriggerHelper<T, ICell<T>> triggerHelper)
    {
      _triggerHelper = triggerHelper;
      typeOf = TriggerType.Always;
    }
